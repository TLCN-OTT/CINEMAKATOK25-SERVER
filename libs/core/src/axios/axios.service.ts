...
export class AxiosService {
    ...
    constructor(){
        ...
        this.axiosInstance = axios.create({
            timeout: config.get('core.axios.timeout'),
            headers:{
    ...
    async postCustom<T>(endpoint: string, data: any, config?: AxiosRequestConfig): Promise<AxiosResponse<T>>{
        try {
            const response: AxiosResponse<T> = await this.axiosInstance.post(endpoint, data, config);
            return response;
        } catch (error) {
    ...
    async putCustom<T>(endpoint: string, data: any, config?: AxiosRequestConfig): Promise<AxiosResponse<T>>{
        try {
            const response: AxiosResponse<T> = await this.axiosInstance.put(endpoint, data, config);
            return response;
        } catch (error) {
